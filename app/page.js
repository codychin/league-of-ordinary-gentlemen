...
<CultureDesk/>
<section className="section upcoming" id="scores"><div className="scoreHero"><div><small>THE BRIEF • SCOREBOARD</small><h2>Week {leagueSnapshot.week}</h2></div><span><b>LIVE</b> SCORES + PROJECTIONS</span></div><div className="upcomingGrid">{currentMatchups.map(([a,b,note],i)=><Link className="matchupLink" href={`/matchups/${matchupPairs[i][0]}/${matchupPairs[i][1]}`} key={i}><article className={note?'featured':''}>{note&&<small>{note}</small>}<div><b>{a.teamName}</b><span className="liveNumbers"><strong>{Number(a.matchup.score).toFixed(1)}</strong><em>PROJ {a.matchup.projection}</em></span></div><i>VS</i><div><b>{b.teamName}</b><span className="liveNumbers"><strong>{Number(b.matchup.score).toFixed(1)}</strong><em>PROJ {b.matchup.projection}</em></span></div><span className="matchupOpen">GAME PREVIEW →</span></article></Link>)}</div></section>
