...
<section className="hero heroStack" aria-label="Featured Hollis Crane investigation">
  <Link className="hero-copy storylink" href="/articles/hollis-arch-manning-compression">
    <div className="photoHero" style={{backgroundImage:"linear-gradient(0deg,rgba(0,0,0,.58),rgba(0,0,0,.06)),url('/api/hollis-arch-compression')",backgroundPosition:"center 42%"}}>
...
<CultureDesk/>
<section className="section upcoming" id="week3"><div className="scoreHero"><div><small>THE BRIEF • NEXT UP</small><h2>Week 3</h2></div><span>PREVIEWS • OPEN THE FILES</span></div><div className="upcomingGrid">{currentMatchups.map(([a,b,note],i)=>{const ap=featuredPlayer(a),bp=featuredPlayer(b);return <Link className="matchupLink" href={`/matchups/${matchupPairs[i][0]}/${matchupPairs[i][1]}`} key={i}><article className="playerHeadlineMatchup">{note&&<small className="matchupKicker">{note}</small>}<div className="matchupPortraits" aria-hidden="true"><img className="leftPlayer" src={playerHeadshot(ap)} alt=""/><img className="rightPlayer" src={playerHeadshot(bp)} alt=""/></div><div className="matchupEditorial"><div className="matchupSide left"><b>{a.teamName}</b><strong>{a.matchup.projection}</strong><em>PROJECTED</em></div><i>VS</i><div className="matchupSide right"><b>{b.teamName}</b><strong>{b.matchup.projection}</strong><em>PROJECTED</em></div></div><div className="matchupPlayers"><span>{ap.name}</span><span>{bp.name}</span></div><span className="matchupOpen">READ WEEK 3 PREVIEW →</span></article></Link>})}</div></section>
<section className="section scoreSection" id="scores"><div className="sectionhead"><div><small className="deskLabel">THE BRIEF • SCOREBOARD</small><h2>Week 2 Final</h2></div><span>FINAL SCORES • MONDAY CLOSED THE BOOK</span></div><div className="scores">{week2Scores.map((s,i)=><div className={`match ${s[4]?'notable':''}`} key={i}>{s[4]&&<small className="scoreNote">{s[4]}</small>}<div><b>{s[0]}</b><strong>{s[1]}</strong></div><div><span>{s[2]}</span><strong>{s[3]}</strong></div></div>)}</div></section>
