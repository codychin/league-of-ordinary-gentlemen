...
const standaloneBoot=`try{var preview=new URLSearchParams(location.search).get('app-preview')==='1';if(preview||matchMedia('(display-mode: standalone)').matches||navigator.standalone===true){document.documentElement.classList.add('standaloneApp');if(preview)document.documentElement.classList.add('appPreview');var p=location.pathname,h=location.hash;document.documentElement.dataset.appTab=p.startsWith('/teams')?'teams':p.startsWith('/matchups')?'scores':p!=='/'?'detail':h==='#scores'?'scores':h==='#culture'?'culture':'home'}}catch(e){}`
