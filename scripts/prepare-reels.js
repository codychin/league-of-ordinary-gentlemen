...
async function main(){
  ...
  for(const [id,url] of Object.entries(reels)){
    ...
    const p=spawnSync(ffmpeg,['-y','-fflags','+genpts','-i',src,'-vf','scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2,fps=30','-c:v','libx264','-profile:v','main','-level:v','3.1','-pix_fmt','yuv420p','-preset','fast','-crf','21','-maxrate','5000k','-bufsize','10000k','-movflags','+faststart','-c:a','aac','-profile:a','aac_low','-b:a','128k','-ar','44100','-ac','2','-shortest',dest],{stdio:'inherit'});
